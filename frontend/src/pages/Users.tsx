...
export function Users() {
  ...
  async function fetchUsers() {
    try {
      ...
      // Ajuste conforme o retorno do fetchUsersController
  ...
  async function handleDeleteUser(id: string, name: string) {
    ...
    try {
      // ✅ AJUSTADO PARA O PARÂMETRO userId DO SEU BACK-END
      await api.delete(`/users/${id}`);
      toast.success('Usuário removido!');
      fetchUsers();
    } catch (error) {
      toast.error('Você não tem permissão ou ocorreu um erro.');
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-surface-600 text-sm">Administração de membros.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative">
             <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-surface-400" size={18} />
             <input type="text" placeholder="Buscar..." className="pl-10 pr-4 py-2 border border-surface-200 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none text-sm w-48 md:w-64" />
          </div>
          {isAdmin && (
            <button className="bg-brand-600 hover:bg-brand-700 text-white px-4 py-2 rounded-lg font-medium transition-all flex items-center gap-2 shadow-sm">
              <UserPlus size={18} /> <span>Novo</span>
            </button>
          )}
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-surface-50 border-b border-surface-200">
              <tr>
                ...
                {isAdmin && <th className="px-6 py-4 text-xs font-bold uppercase text-surface-500 text-right">Ações</th>}
              </tr>
            </thead>
            <tbody className="divide-y divide-surface-100">
              {loading ? <UserTableSkeleton /> : users.map((u) => (
